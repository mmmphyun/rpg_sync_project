...
def delete_tip_by_id(tip_id: int) -> int:
    """팁 게시글 Soft Delete"""
    sql = "UPDATE tips SET is_deleted = TRUE, title = '', content = '', updated_at = CURRENT_TIMESTAMP WHERE tip_id = %s;"
    conn = get_connection()
    cursor = conn.cursor()
    try:
...
def delete_comment_by_id(comment_id: int) -> int:
    """댓글 Soft Delete"""
    sql = "UPDATE tip_comments SET is_deleted = TRUE, content = '', updated_at = CURRENT_TIMESTAMP WHERE comment_id = %s;"
    conn = get_connection()
    cursor = conn.cursor()
    try:
